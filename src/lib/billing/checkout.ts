...
export async function createCheckoutSession({
  ...
  // 1. Fetch full user to check for existing Stripe Customer ID if user is provided
  let user = null;
  if (sessionUser?.id) {
  ...
  // 3. Check for existing active subscriptions (only if we have a customer)
    ...
    try {
      ...
      const activeSub = subscriptions.data.find(
        (sub) =>
          ...
          sub.metadata?.siteSlug === siteSlug
      ...
      if (error.code === 'resource_missing' && error.param === 'customer' && sessionUser) {
  ...
  try {
    ...
    if (
      ...
      stripeError.param === "customer" && sessionUser
    ) {
      console.warn(
