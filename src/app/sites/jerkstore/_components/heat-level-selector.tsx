...
import { Baby, Flame, Skull } from "lucide-react";
...
export function HeatLevelSelector({ heatLevel, setHeatLevel, className }: HeatLevelSelectorProps) {
      ...
      {HEAT_LEVELS.map(level => {
        ...
        return (
          <button
            key={level.value}
            type="button"
            onClick={() => setHeatLevel(level.value as HeatLevel)}
            title={`${level.label} - ${level.desc}`}
            className={cn(
              "w-8 h-8 sm:w-10 sm:h-10 border-2 border-black transition-all duration-100 flex items-center justify-center",
              isSelected
